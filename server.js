...
io.sockets.on('connection', function(socket) {
	...
	socket.on('add name', function(data) {
		...
		if(DAY_STARTED) { //daytime has started
			...
			if(PLAYER_LIST[socket.id].index - 1 != index) { //can't be yourself
	...
	if(CURRENT_ROLE_INDEX < NUM_ROLES) { //day has not begun yet
		...
		if(CURRENT_ROLE_INDEX + 1 > NUM_ROLES) {
			...
			setTimeout(function() {
				...
				//if the vote is empty, VOTE TO THE RIGHT (+1)
				for(var i in SOCKET_LIST) { //get votes
					if(kill_list[SOCKET_LIST[i].vote - 1] === ' ') {
						//make vote (index + 1) % NUM_PLAYERS
						//don't allow vote for center cards
					}
					else {
						kill_list[SOCKET_LIST[i].vote - 1] += 1;
					}
			...
			}, day_timer); //30 seconds, or 300??
		}
	}
	else { //DAY TIME START
		var current_time = new Date().getTime() / 1000;
		timer = (Math.round((day_timer / 10) - (current_time - start_timer) * 100)) / 100;
		if(timer < 0)
			timer = '0.00';
		for(var i in SOCKET_LIST) {
			var socket = SOCKET_LIST[i];
